...
densidad_por_item <- function(data, filePrefix = "", bw = "nrd0", minx = NULL, maxx = NULL) {
  ...
  g <- ggplot(mostCommonItems, aes(x = Distancia, fill = Item)) + geom_density(alpha = 0.25, bw = bw) + xlim(c(minx, maxx))
  # -----------
...
runStats = function(directory, distanceFile = "distances.txt") {
  ...
  ### ----------------------------------------------------- JONNO
  # mostCommonItems <- subset(extract_most_common_items(data, topN), Distancia < maxx)
  mostCommonItems <- extract_most_common_items(items, topN)
  # mostCommonItems <- extract_most_common_not_grouped(items, topN)	# ORIG!
  ### -----------------------------------------------------
